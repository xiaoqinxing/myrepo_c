...
{
    ASSERT(str != NULL, "error: invalid string request");

    buf_capacity = strlen(str) + 1;
    buf = (char*)malloc(buf_capacity);
    COMMON_ASSERT(buf != NULL, "error: allocation fail");
...
{
    COMMON_ASSERT(capacity != 0, "error: invalid string request");
...
{
    COMMON_ASSERT(pos < capacity(), "error: invalid string request");
...
{
    COMMON_ASSERT(!(UINT_MAX - len < buf_len), "error: Overflow");
    //try to resize the buffer, if needed
    if (buf_len + len >= buf_capacity) {
        char *buf_temp = NULL;
        if (0 == buf_len) {
            buf_temp = (char*)malloc(len + 1);
            COMMON_ASSERT(buf_temp != NULL, "error: allocation fail");
            buf_capacity = len + 1;
        } else {
...
{
    COMMON_ASSERT(pos < size(), "error: arg out of range");
