...
void* af_thread_handler(void *af_data)
{
  ...
  do {
    ...
    case MSG_AF_START: {
      ...
      /* Copy custom data into output */
      output.af_custom_param = camera_af->af_obj->output.af_custom_param;

      ATRACE_CAMSCOPE_BEGIN(CAMSCOPE_AF_START);
      af_ops.set_parameters(&msg->u.af_set_parm, &output, 1, af_algo_obj);

      /* Copy back output data */
      output.af_custom_param =
        camera_af->af_obj->output.af_custom_param;
      camera_af->af_obj->output = output;
    ...
    case MSG_AF_SET: {
      ...
      if (af_ops.set_parameters(&msg->u.af_set_parm, &output, 1, af_algo_obj)) {
        /* Copy back output data */
        output.af_custom_param = camera_af->af_obj->output.af_custom_param;
        camera_af->af_obj->output = output;
    ...
    case MSG_BF_STATS: {
      ...
      if (!af_thread_ctrl->no_stats_mode) {
        ...
        af_ops.process(msg->u.stats, &output, 1, af_algo_obj);
